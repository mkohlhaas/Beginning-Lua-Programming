...
-- Returns an iterator that iterates through all self's elements.
-- (This is added later in the chapter.)
function Methods:Elems()
  local IterPos -- The position of the element the iterator needs to return
  return function()
    local Ret
    if IterPos then
      if IterPos ~= self.Pos then
        ...
        -- Back at the beginning; do nothing (which ends the loop by returning nil).
      end
    else
      -- At the beginning: initialize IterPos:
      IterPos = self.Pos
      Ret = self[IterPos] -- If the ring is empty, this'll end the loop by returning nil.
    end
