...
--
-- Usage: lua sortuniq.lua INFILE OUTFILE
--
...
function Main(InFilename, OutFilename)
  -- Make the lines of the input file (standard input if no name was given) keys of a table:
  local Lines = {}
  local Iter = InFilename and io.lines(InFilename) or io.lines()
  for Line in Iter do
    Lines[Line] = true
  end
  -- Get a handle to the output file (standard output if no name was given):
...
-- $ lua sortuniq.lua < testin.txt > testout.txt
--
-- Should create testout.txt with:
-- alpha
-- bravo
-- charlie
