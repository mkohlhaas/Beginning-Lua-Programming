-- "Ordered tables" -- they remember the order in which things are put into them.

local RealTbls = {} -- Keys: proxy tables; values: the real tables.
local NumToKeys = {} -- Keys: proxy tables; values: arrays of real tables' keys, in order.
local KeyToNums = {} -- Keys: proxy tables; values: tables mapping real tables' keys to their (order) numbers.
-- Prevent leakage by giving all of the above tables weak keys (this will be explained in Chapter 10):
local ModeMeta = {__mode = "k"}
setmetatable(RealTbls, ModeMeta)
setmetatable(NumToKeys, ModeMeta)
setmetatable(KeyToNums, ModeMeta)

-- Does the bookkeeping necessary to add a key and its order to the real table (or update/delete a key):
local function __newindex(Proxy, Key, Val)
  ...
  if RealTbl[Key] == nil then
    -- This is a new key.  Only add it if the value's non-nil:
    if Val ~= nil then
      -- Record the value:
      RealTbl[Key] = Val
      -- Record the order:
    ...
    if Val ~= nil then
      ...
      KeyToNum[Key] = nil
      -- table.remove will shift down anything in NumToKey that's higher than Num, but it needs to be done by hand for KeyToNum:
...
-- An iterator that iterates through all the real table's key-value pairs in the correct order:
...
function pairs(Tbl)
  -- The real pairs only needs to be replaced because it returns its own copy of next unaffected by our replacement.
...
-- Returns a table that remembers the order in which keys are added to it:
function MakeOrderedTbl()
  local RealTbl, Proxy = {}, {}
  RealTbls[Proxy] = RealTbl
  -- The following two tables are two complementary ways of recording the order that keys are added in:
  NumToKeys[Proxy] = {}
  KeyToNums[Proxy] = {}
